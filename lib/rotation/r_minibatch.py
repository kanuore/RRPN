...
def r_get_rotate_minibatch(roidb, num_classes):
    ...
    # generate random number of angle of images 
    # to argument the dataset with type of anti-clockwise angle
    # 参数化那些负角度
...
def rotate_gt_bbox(origin_gt_roidb, angle, gt_inds):
    ...
    # print 'build rotated_gt_boxes : ',rotated_gt_boxes
    ...
    #print 'before : ',origin_gt_boxes[:, 0:2]
    ...
    #print "after : ",origin_gt_boxes[:, 0:2]
...
def r_get_minibatch(roidb, num_classes):
    ...
    num_images = len(roidb)
    # Sample random scales to use for each image in this batch
    # [low, high)之间的随机整数, size是数目，  [0]
    random_scale_inds = npr.randint(0, high=len(cfg.TRAIN.SCALES),
                                    size=num_images)
    assert(cfg.TRAIN.BATCH_SIZE % num_images == 0), \
        'num_images ({}) must divide BATCH_SIZE ({})'. \
        format(num_images, cfg.TRAIN.BATCH_SIZE)
    #大胆猜测一下，一个batch提供BATTCH_SIZE个roi，平均分到这个batch中的每张
    rois_per_image = cfg.TRAIN.BATCH_SIZE / num_images
    fg_rois_per_image = np.round(cfg.TRAIN.FG_FRACTION * rois_per_image)

    # 这里设置scale，将原图缩放到设定的几种scale
    # Get the input image blob, formatted for caffe
    ...
    # bbox: [ctr_x, ctr_y, height, width, angle]
    # print __file__,__name__,'HAS_RPN',cfg.TRAIN.HAS_RPN
...
def _get_image_blob(roidb, scale_inds):
    ...
    for i in xrange(num_images):
        ...
        #print __file__,'==>im : ',im.shape
        #print __file__,'==>scale : ',im_scale
    # Create a blob to hold the input images
    blob = im_list_to_blob(processed_ims)
