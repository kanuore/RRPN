...
def _get_image_blob(im):
    ...
    """
    # im_scale_factors应该将原图缩放至不同大小进行测试
...
def r_im_detect(net, im, boxes=None):
    ...
    # blob_keys :  ['rois', 'data']
    # blob['data'] :  (1, 3, 1000, 1000)
    blobs, im_scales = _get_blobs(im, boxes)
    # print "blob : ",blobs['data'].shape
    # print "blob : ",blobs['rois'].shape
    # print "im_scales : ",im_scales.shape
    ...
    # im_blob [num_batch,channels,height,weight]
    if cfg.TEST.HAS_RPN:
        im_blob = blobs['data']
    ...
    # blobs_out 网络输出['bbox_pred', 'cls_prob']
    # bbox_pred :  (300, 10)
    # cls_prob : (300, 2)
    blobs_out = net.forward(**forward_kwargs)
    print "blobs_out : ",blobs_out.keys()
    print "bbox_pred : ",blobs_out['bbox_pred'].shape
    print "cls_prob : ",blobs_out['cls_prob'].shape  
    scores = net.blobs['rpn_cls_prob_reshape']
    # feature map anchor visualization
    ...
    if False: 
        ...
        print 'data.blob : ',net.blobs['data'].data.shape
        # RPN中的部分
        # rpn_cls_prob_reshape [num_batch,108(6*3*3*5),32,32]
        # 单这句 取54：108是什么意思,
        # 应该是取一半的意思吧，这样的话，前后景的排列形式就得是，前景s + 后景s
    ...
    #1
        ...
        # 角度转弧度，角度并不是体现在值上，而是体现在索引上，
        # 6*size_1_scale_1 + 6*size_1_scale_2 + ... + 6*size_3_scale_3 
    ...
    # feature map proposal visualization
    if False: 
        # 800*1067
        # 800/16 = 50
        # 1067/16 = 67
    ...
    # 如果模型中使用到RPN网络，则从网络中获取rois
    if cfg.TEST.HAS_RPN:
        assert len(im_scales) == 1, "Only single-image batch implemented"
        # 这里是根据RPN的二分类结果，参照im_info生成的一些框的信息
        rois = net.blobs['rois'].data.copy()
        # rois.shape (300, 6)
        print 'rois.shape',rois.shape
        # unscale back to raw image space（原始图像）
        # 1：6（12345）
