...
def get_rroidb(mode):   
    ...
    L_DATASET = []
    # 返回img_infos
    #L_ICDAR2003_TRAIN = get_ICDAR2003(mode)
    L_ICDAR2003_TRAIN = get_LINES(mode)
    print "L_ICDAR2003_TRAIN:", len(L_ICDAR2003_TRAIN)
    
    # 将img_infos加入列表，相当于list.append()
...
def get_ICDAR2003(mode):
    ...
    for gt_image in gt_images:
        ...
        for box_node in box_nodes:
            ...
            boxes.append([x_ctr, y_ctr, height, width, angle])
        # 查看数据
        # vis_image(DATASET_DIR+image_file,boxes)
...
def get_LINES(mode):
    ...
    DATASET_DIR = "/media/lbk/ubuntu file/dataset/lines/"

    gt_xml_file = "lines_data.xml"

    gt_tree = xml.dom.minidom.parse('/home/lbk/ocr/RRPN-master/data/demo/test_line_data/20.xml'
        )
    ...
    for gt_image in gt_images:
        ...
        for box_node in box_nodes:
            width = int(box_node.getAttribute("width"))
            height = int(box_node.getAttribute("height"))
            x_ctr = int(int(box_node.getAttribute("x")) + width / 2.0)
            y_ctr = int(int(box_node.getAttribute("y")) + height / 2.0)
            angle = 0
            #angle = float(box_node.getAttribute("rotation")) # rectangle
            # 如果这里按”全局逆时针“更改角度
            # if angle < -30:
