...
#all_anchor:[x,y,h,w],所以左上角应该为- all_anchors[:, 3] / 2, - all_anchors[:, 2] / 2
#角度以‘逆时针’，水平0度为准
...
def pts_dot(pts, rotat_matrix):
    ...
    return np.array([pts[:, 0] * rotat_matrix[0] + pts[:, 1] * rotat_matrix[2], 
                        pts[:, 0] * rotat_matrix[1] + pts[:, 1] * rotat_matrix[3]]).T
